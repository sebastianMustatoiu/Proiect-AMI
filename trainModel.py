...
genuri_muzicale = ["manea", "manea", "trap", "trap", "manea", "trap", "trap"]

cuvinte_fara_stopwords_lista = procesare_versuri_fara_stopwords(versuri)
versuri_melodii, genuri_muzicale = zip(*[(versuri[i], genuri_muzicale[i]) for i in range(len(versuri))])

model = make_pipeline(CountVectorizer(), MultinomialNB())
skf = StratifiedKFold(n_splits=3)
scores = cross_val_score(model, versuri_melodii, genuri_muzicale, cv=skf)
print("Acuratețea pentru fiecare iterație de cross-validation:", scores)
print("Acuratețea medie:", scores.mean())

versuri_train, versuri_test, genuri_train, genuri_test = train_test_split(versuri, genuri_muzicale, test_size=0.15, random_state=42)
model.fit(versuri_train, genuri_train)
pred = model.predict(versuri_test)

print("Raport de clasificare pentru identificarea genului muzical:")
print(classification_report(genuri_test, pred, zero_division=0))
